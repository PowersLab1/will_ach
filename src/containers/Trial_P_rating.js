...
import { add_array, add_response_P, add_response_time_P_rating, add_contrast_P_rating, add_rating_P_rating } from '../actions/data'
    ...
    constructor(props) {
      ...
      create_noise( audioContext, callback ){
        ...
        async function generate_noise( ){
            ...
            if (that.state.responses_P_rating.length > 0){
              console.log('index: ' + index);
              if (that.state.responses_P_rating[(index-1)] == check[(index-1)]){
                console.log("correct");
                that.state.correct++;
            ...
            index++;
              ...
              console.log(that.state.responses_P_rating, that.state.contrast_array_P_rating);

              stimulus = createGabor( stim, contrast );
              beep(50, 830, 350, audioContext); //beep for arg3 miliseconds
            }
            ...
            for (var x = 0; x < 256; x++) {
              for (var y = 0; y < 256; y++) {
                if(t == start_time + jitter || t == start_time + jitter + 1 || t == start_time + jitter + 2|| t == start_time+ jitter+3 || t == start_time + jitter+4
                  ...
                  if( that.state.responses_P_rating.length == (that.state.ratings_P_rating.length + 1) ) {
                  ...
                  if(that.state.time_window_start + 2 == seconds){
                    ...
                    if(that.state.contrast_array_P_rating.length < that.state.responses_P_rating.length){
                      that.setState({ contrast_array_P_rating: that.state.contrast_array_P_rating.concat([contrast]) });
                    }

                    if( that.state.responses_P_rating.length != that.state.ratings_P_rating.length ){

                      if( that.state.responses_P_rating.length > that.state.ratings_P_rating.length ){

                        console.log("response and response time discarded", that.state.responses_P_rating, that.state.response_time_P_rating, that.state.ratings_P_rating, that.state.contrast_array_P_rating);

                        that.setState({
                          responses_P_rating: that.state.responses_P_rating.slice(0, that.state.ratings_P_rating.length),
                          response_time_P_rating: that.state.response_time_P_rating.slice(0, that.state.ratings_P_rating.length),
                          counter: that.state.counter - 1,
                          contrast_array_P_rating: that.state.contrast_array_P_rating.slice(0, that.state.ratings_P_rating.length)
                        });
                      }

                      else{
                        console.log("rating discarded", that.state.responses_P_rating, that.state.ratings_P_rating, that.state.contrast_array_P_rating);

                        that.setState({
                          ratings_P_rating: that.state.ratings_P_rating.slice(0, that.state.responses_P_rating.length),
                        });
                      }
                    }
           ...
           var yPos = (canvas.height/2) - (recHeight/2);
            ...
            if(index == end ){
              ...
              for(var g = 0; g < end; g++){
                that.props.add_response_P( that.state.responses_P_rating[g] );
                that.props.add_response_time_P_rating( that.state.response_time_P_rating[g] );
                that.props.add_contrast_P_rating( that.state.contrast_array_P_rating[g] );
                that.props.add_rating_P_rating( that.state.ratings_P_rating[g] );
      ...
      keyFunction(event){
        ...
        if(event.keyCode === 81 && this.state.time_window == true){
          ...
          this.setState({
            ...
            responses_P_rating: this.state.responses_P_rating.concat([1]),
            response_time_P_rating: this.state.response_time_P_rating.concat([ seconds - this.state.time_window_start]),
            time_window: false,
          });
        ...
        if(event.keyCode === 69 && this.state.time_window == true) {
          this.setState({
            counter: this.state.counter + 1,
            responses_P_rating: this.state.responses_P_rating.concat([0]),
            response_time_P_rating: this.state.response_time_P_rating.concat([ seconds - this.state.time_window_start]),
            time_window: false,
          });
        ...
        if( ( event.keyCode === 49 || event.keyCode === 50 || event.keyCode === 51 || event.keyCode === 52 || event.keyCode === 53) && this.state.time_window_rating == true){
          this.setState({
            ratings_P_rating: this.state.ratings_P_rating.concat([event.keyCode - 48]),
            time_window_rating: false,
          });
        }
      }
...
const mapDispatchToProps = dispatch => ({
  ...
  add_response_time_P_rating: ( element ) => dispatch(add_response_time_P_rating( element )),
  add_contrast_P_rating: ( element ) => dispatch(add_contrast_P_rating( element )),
  add_rating_P_rating: ( element ) => dispatch(add_rating_P_rating( element )),
  add_array: ( arr ) => dispatch(add_array( arr )),
})

export default connect(mapStateToProps, mapDispatchToProps)(Trial_P_rating);
