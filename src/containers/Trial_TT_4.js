...
import { add_array, add_response_tt_4, add_response_time_tt_4, add_contrast_tt_4, add_rating_tt_4} from '../actions/data'
      ...
      create_noise( audioContext, callback ){
        ...
        async function generate_noise( ){
            ...
            if ( t == start_time + jitter){
              ...
              console.log(that.state.responses_tt_4, that.state.ratings_tt_4, that.state.response_time_tt_4, that.state.contrast_array_tt_4);
              ...
              for (var y = 0; y < 256; y++) {
                if(t == start_time + jitter || t == start_time + jitter + 1 || t == start_time + jitter + 2|| t == start_time+ jitter+3 || t == start_time + jitter+4
                  ...
                  if(that.state.time_window_start + 2 == seconds){
                    ...
                    if(that.state.contrast_array_tt_4.length < that.state.responses_tt_4.length){
                      that.setState({ contrast_array_tt_4: that.state.contrast_array_tt_4.concat([contrast]) });
                    }

                    if( that.state.responses_tt_4.length != that.state.ratings_tt_4.length ){

                      if( that.state.responses_tt_4.length > that.state.ratings_tt_4.length ){

                        console.log("response and response time discarded", that.state.responses_tt_4, that.state.response_time_tt_4, that.state.ratings_tt_4, that.state.contrast_array_tt_4);

                        that.setState({
                          responses_tt_4: that.state.responses_tt_4.slice(0, that.state.ratings_tt_4.length),
                          response_time_tt_4: that.state.response_time_tt_4.slice(0, that.state.ratings_tt_4.length),
                          counter: that.state.counter - 1,
                          contrast_array_tt_4: that.state.contrast_array_tt_4.slice(0, that.state.ratings_tt_4.length)
                        });
                      }

                      else{
                        console.log("rating discarded", that.state.responses_tt_4, that.state.ratings_tt_4, that.state.contrast_array_tt_4);

                        that.setState({
                          ratings_tt_4: that.state.ratings_tt_4.slice(0, that.state.responses_tt_4.length),
                        });
                      }
                    }
            ...
            if(block_index == block_end && index == end ){
              ...
              for(var g = 0; g < end; g++){
                that.props.add_response_tt_4( that.state.responses_tt_4[g] );
                that.props.add_response_time_tt_4( that.state.response_time_tt_4[g] );
                that.props.add_contrast_tt_4( that.state.contrast_array_tt_4[g] );
                that.props.add_rating_tt_4( that.state.ratings_tt_4[g] );                
              }
      ...
      keyFunction(event){
        ...
        if(event.keyCode === 81 && this.state.time_window == true){
          ...
          this.setState({
            ...
            responses_tt_4: this.state.responses_tt_4.concat([1]),
            response_time_tt_4: this.state.response_time_tt_4.concat([ seconds - this.state.time_window_start]),
            time_window: false,
          });
        ...
        if(event.keyCode === 69 && this.state.time_window == true) {
          this.setState({
            counter: this.state.counter + 1,
            responses_tt_4: this.state.responses_tt_4.concat([0]),
            response_time_tt_4: this.state.response_time_tt_4.concat([ seconds - this.state.time_window_start]),
            time_window: false,
          });
        ...
        if( ( event.keyCode === 49 || event.keyCode === 50 || event.keyCode === 51 || event.keyCode === 52 || event.keyCode === 53) && this.state.time_window_rating == true){
          this.setState({
            ratings_tt_4: this.state.ratings_tt_4.concat([event.keyCode - 48]),
            time_window_rating: false,
          });
        }
      }
...
const mapDispatchToProps = dispatch => ({
  add_response_tt_4: ( element ) => dispatch(add_response_tt_4( element )),
  add_response_time_tt_4: ( element ) => dispatch(add_response_time_tt_4( element )),
  add_contrast_tt_4: ( element ) => dispatch(add_contrast_tt_4( element )),
  add_rating_tt_4: ( element ) => dispatch(add_rating_tt_4( element )),
  add_array: ( arr ) => dispatch(add_array( arr )),
})

export default connect(mapStateToProps, mapDispatchToProps)(Trial_TT_4);
