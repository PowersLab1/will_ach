...
import './Trial_TT_2.css';
import { Redirect } from "react-router-dom";
import { createStim, createGabor } from "../Stim.js"
import {create_blocks} from "../tt_blocks.js"

import rating_scale from "../media/rating_scale.png"
import rating_1 from "../media/rating_1.png"
import rating_2 from "../media/rating_2.png"
import rating_3 from "../media/rating_3.png"
import rating_4 from "../media/rating_4.png"
import rating_5 from "../media/rating_5.png"

import { connect } from 'react-redux'
import { add_array, add_response_tt_2, add_response_time_tt_2, add_contrast_tt_2, add_rating_tt_2 } from '../actions/data'
      ...
      create_noise( audioContext, callback ){
        ...
        async function generate_noise( ){
            ...
            if ( t == start_time + jitter){
              ...
              console.log(that.state.responses_tt_2, that.state.ratings_tt_2, that.state.response_time_tt_2, that.state.contrast_array_tt_2);
            ...
            if(that.state.responses_tt_1.length == (that.state.ratings_tt_1.length +1) ) {
                  ...
                  if(that.state.time_window_start + 2 == seconds){
                    ...
                    if(that.state.contrast_array_tt_2.length < that.state.responses_tt_2.length){
                      that.setState({ contrast_array_tt_2: that.state.contrast_array_tt_2.concat([contrast]) });
                    }

                    if( that.state.responses_tt_2.length != that.state.ratings_tt_2.length ){

                      if( that.state.responses_tt_2.length > that.state.ratings_tt_2.length ){

                        console.log("response and response time discarded", that.state.responses_tt_2, that.state.response_time_tt_2, that.state.ratings_tt_2, that.state.contrast_array_tt_2);

                        that.setState({
                          responses_tt_2: that.state.responses_tt_2.slice(0, that.state.ratings_tt_2.length),
                          response_time_tt_2: that.state.response_time_tt_2.slice(0, that.state.ratings_tt_2.length),
                          counter: that.state.counter - 1,
                          contrast_array_tt_2: that.state.contrast_array_tt_2.slice(0, that.state.ratings_tt_2.length)
                        });
                      }

                      else{
                        console.log("rating discarded", that.state.responses_tt_2, that.state.ratings_tt_2, that.state.contrast_array_tt_2);

                        that.setState({
                          ratings_tt_2: that.state.ratings_tt_2.slice(0, that.state.responses_tt_2.length),
                        });
                      }
                    }
            ...
            if(that.state.responses_tt_1.length == (that.state.ratings_tt_1.length +1) ) {
              if (that.state.time_window_start < seconds && that.state.time_window_start > seconds - 2) {
            ...
            if(block_index == block_end && index == end ){
              ...
              for(var g = 0; g < end; g++){
                that.props.add_response_tt_2( that.state.responses_tt_2[g] );
                that.props.add_response_time_tt_2( that.state.response_time_tt_2[g] );
                that.props.add_contrast_tt_2( that.state.contrast_array_tt_2[g] );
                that.props.add_rating_tt_2( that.state.ratings_tt_2[g] );
              }
      ...
      keyFunction(event){
        ...
        if(event.keyCode === 81 && this.state.time_window == true){
          ...
          this.setState({
            ...
            responses_tt_2: this.state.responses_tt_2.concat([1]),
            response_time_tt_2: this.state.response_time_tt_2.concat([ seconds - this.state.time_window_start]),
            time_window: false,
          });
        ...
        if(event.keyCode === 69 && this.state.time_window == true) {
          this.setState({
            counter: this.state.counter + 1,
            responses_tt_2: this.state.responses_tt_2.concat([0]),
            response_time_tt_2: this.state.response_time_tt_2.concat([ seconds - this.state.time_window_start]),
            time_window: false,
          });
        ...
        if( ( event.keyCode === 49 || event.keyCode === 50 || event.keyCode === 51 || event.keyCode === 52 || event.keyCode === 53) && this.state.time_window_rating == true){
          this.setState({
            ratings_tt_2: this.state.ratings_tt_2.concat([event.keyCode - 48]),
            time_window_rating: false,
          });
        }
      }
...
const mapDispatchToProps = dispatch => ({
  add_response_tt_2: ( element ) => dispatch(add_response_tt_2( element )),
  add_response_time_tt_2: ( element ) => dispatch(add_response_time_tt_2( element )),
  add_contrast_tt_2: ( element ) => dispatch(add_contrast_tt_2( element )),
  add_rating_tt_2: ( element ) => dispatch(add_rating_tt_2( element )),
  add_array: ( arr ) => dispatch(add_array( arr )),
})

export default connect(mapStateToProps, mapDispatchToProps)(Trial_TT_2);
